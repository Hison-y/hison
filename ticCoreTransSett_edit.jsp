...
/*     function   utf8CodeToChineseChar(strUtf8) 
    ...
    function selectFunction() {
    		...
    		Dialog_TreeList(false,"fdFuncBaseId","fdFuncBaseName",";",
    			           "ticCoreFindFunctionService&selectId=!{value}&type=cate&fdAppType=${param.fdAppType}",
    			           "业务分类",
    			           "ticCoreFindFunctionService&selectId=!{value}&type=func&fdAppType=${param.fdAppType}&noTrans=true",
    			           after_dialogSelect,
    			           "ticCoreFindFunctionService&type=search&keyword=!{keyword}&fdAppType=${param.fdAppType}&noTrans=true",
    			           null,null,null,
    			           "选择函数");
   }
    ...
    $(function(){
    	...
    	//测试数据
    	//$("#fdTransSett_json").val('{"import":[{"name":"T2","type":"string","title":"123","display_value":"23453245","hidden_value":"23453245"},{"name":"T4","type":"object","title":"T4","children":[{"name":"EMPID","type":"string","title":"业务事件的对象标识已提供","display_value":"2 / 3 * 6","hidden_value":"2 / 3 * 6"},{"name":"NAME9","type":"string","title":"雇员的姓","display_value":"$testName$","hidden_value":"$testLabel$"}]},{"name":"ZBAPI_TIB_1_PARA","type":"object","title":"ZBAPI_TIB_1_PARA","children":[{"name":"EMPID","type":"string","title":"业务事件的对象标识已提供","display_value":"$testName$ / 3","hidden_value":"$testLabel$ / 3"},{"name":"NAME9","type":"string","title":"雇员的姓","display_value":"345345","hidden_value":"345345"}]},{"name":"T3","type":"array","title":"T3","children":[{"name":"TEXT","type":"string","title":"一条信息的文本行","display_value":"$testName$ / 3","hidden_value":"$testLabel$ / 3"}]}],"export":[{"title":"RETURN","name":"RETURN","children":[{"title":"消息类型: S 成功,E 错误,W 警告,I 信息,A 中断","isoptional":"false","name":"TYPE","length":"1","type":"string"},{"title":"消息代码","isoptional":"false","name":"CODE","length":"5","type":"string"},{"title":"消息文本","isoptional":"false","name":"MESSAGE","length":"220","type":"string"},{"title":"应用程序日志: 日志号","isoptional":"false","name":"LOG_NO","length":"20","type":"string"},{"title":"应用日志：内部邮件序列号","isoptional":"false","name":"LOG_MSG_NO","length":"6","type":"string"},{"title":"消息变量","isoptional":"false","name":"MESSAGE_V1","length":"50","type":"string"},{"title":"消息变量","isoptional":"false","name":"MESSAGE_V2","length":"50","type":"string"},{"title":"消息变量","isoptional":"false","name":"MESSAGE_V3","length":"50","type":"string"},{"title":"消息变量","isoptional":"false","name":"MESSAGE_V4","length":"50","type":"string"}],"type":"object"}],"trans_info":{"trans_type":"1","display_value":"23 / 6","hidden_value":"23 / 6"}}');
    	//初始化值,编辑页面时值的回显
    	//var fdTransSettStr= $("#fdTransSett_json").val();
    	init();
    	...
    	$("#is_all_trans_out_img").click(function(){
			mapping('all_trans_display_out','all_trans_hidden_out','in');
    	});
    });
    ...
    function buildParaInTable(trs){
    	for(var i=0;i<trs.length;i++){
			...
			{
				tr_new = $("<tr id='"+tr.id+"'><td>"+tr.name+"</td><td>"+tr.title+"</td><td><textarea style='width: 180px;height: 25px' readonly='readonly' name='"+tr.id+"_input'></textarea><input type='hidden' name='"+tr.id+"_hidden'></input></td><td align='center'><img id='"+tr.id+"_img' class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0'></td></tr>");
			}
			...
			if(tr.display_value){
	        	...
	        	$($("#"+tr.id).find('input')[0]).val(hidden_value.replace(/\\n/g,"\n"));
			}
    ...
    function buildBusinessExceptionParamSelect(trs){
    	...
    	for(i=0;i<trs.length;i++){
    		var tr = trs[i];
    		if(tr.child){//如果有子节点的放入map中
    			parentMap.set(tr.id,tr);
    		}else{
    			childArray[index] = tr;
    			index++;
    		}
    	...
    	for(i=0;i<childArray.length;i++){
    		var nameStr = recursionGetParentName(childArray[i].name,childArray[i].parentId,parentMap);
    		var option = "<option value ='" +nameStr+ "' >"
			+nameStr + "</option>";
			options.push(option);
    	}
    ...
    function recursionGetValueToJson(inParamJson,rtnParamJson,id)
    {
    	...
    	}else
    	{
        	...
        	var hiddenValue = $($("#"+id.replace(/:/g,"_")).find('input')[0]).val();
    ...
    function addField(type,parent){
		...
		if(type=="out"){
			...
			td_value = "<td align='center'><input type='checkbox' name='isSearch'/></td><td><textarea style=\"width: 180px;height: 25px\" readonly=\"readonly\" name='"+id+"_input' class='para_value'></textarea><input type='hidden' name='"+id+"_hidden' class='para_value_hidden'></td>";
  ...
  /*   function genSelectStr(valueType){
    ...
    function addFieldFromForm(type,parentName,objName,name,valueType,display_value,hidden_value,hierId,isSearch){
		...
		if(type=="out"){
			...
			td_value = "<td>"+input_search+"</td><td><textarea style=\"width: 180px;height: 25px\" readonly=\"readonly\" name='"+id+"_input' class='para_value'>"+display_value_str+"</textarea><input type='hidden' name='"+id+"_hidden' class='para_value_hidden' value='"+hidden_value_str+"'></td>";
			a_edit = " <img  src=\"${KMSS_Parameter_StylePath}icons/edit.gif\" onClick=\"mapping('"+id+"_input','"+id+"_hidden','out');\" class='a_edit' title='编辑'/>";
		}
    ...
    function deleteField(obj){
    	$(obj).parent().parent().siblings("tr.child-of-" + $(obj).parent().parent().attr("id")).each(function(index) {
    			$(this).remove();
    	...
    	if(parent){
        	...
        	for(var i=0;i<inputs.length;i++){
        		if($(inputs[i]).val()==parent){
        			var children = $(inputs[i]).parent().parent().siblings("tr.child-of-" + $(inputs[i]).parent().parent().attr("id"));
...
function  after_updateTicCoreTransSett(rtn){
	...
	var data = rtn.GetHashMapArray();
	if(data){
		var paraIn=JSON.parse(data[0]["paraIn"]);
		var paraOut=JSON.parse(data[0]["paraOut"]);
	}
	trs = new Array();
	importArray=[];
	$.each(paraIn, function(idx, obj) {
	...
	$.each(importArray, function() {
	     ...
	     $("#table_paraIn").find("input[name="+this.name+"_hidden]").val(this.hidden_value);
	});
}
...
function formulaAdaptOut(transOut,sourceExport,parentName,parentTitle){
	if(transOut && sourceExport){
		...
		if(sourceExport.name == name || sourceExport.title==title){
			...
			$("#"+tr_id).find("textarea").val(value_display);
			$("#"+tr_id).find("input:hidden").val(value_hidden);
		}
	}
	}
}
...
function formulaAdapt(tableName){
	...
	if(tableName=='table_paraOut_trans'){
		if(exportInfo && transOut && exportInfo.length>0 && transOut.length>0){
			formulaAdaptOut(transOut[0],exportInfo[0],exportInfo[0].name,exportInfo[0].title);
		}
		return ;
	}
	 transInArray=[];
	 ...
	 for(var i=0;i< importInfoArray.length;i++){
		 ...
		 transInArray.find(function(value) {
			 if(value ==name) {
			      ...
			      $("#table_paraIn").find("input[name="+hiddenName+"]").val(name);
			 }
		})
	 }
}
            		...
            		<div style="width:100%">
						...
						<div style="width:100%;margin-top: 5px" >
							<table class="tb_normal" style="width:100%">
								...
								<tr>
									<td align="center"><select id="businessExceptionParamSelect" name="fdReturnValue" onchange=""></select>
									<div align="center" >
										<bean:message bundle="tic-sap-connector"
										key="ticSapRfcSetting.lang.return" />
										 <input type="text"
										name="fdSuccess"
										value="${ticCoreTransSettForm.fdSuccess}" class="inputsgl">
										<bean:message
										bundle="tic-sap-connector"
